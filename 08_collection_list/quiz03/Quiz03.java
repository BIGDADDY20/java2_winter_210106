...
public class Quiz03 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < newNames.size(); i++) {
			String newName = newNames.get(i); // 검사할 새이름
			
			if (isSame) { // 동명이인이면,
				newName = newNames.get(i) + count++;  // 이름+번호가 붙은 이름으로 교체 후 다시 검사한다
			}
			isSame = false; // 초기화
			
			for (String name : names) {
				if (newName.equals(name)) { // 동명이인
					isSame = true;
					i--; // 이름+번호가 붙은 것으로 한번 더 검사하기 위해
					break;
				}
			}
			
			if (!isSame) { // 동명이인 아니면 이름 그대로 저장
				names.add(newName); 
				count = 1; // 초기화
			}
		}
